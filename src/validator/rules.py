...
and returns every finding in stable source order; rules share no mutable state
so the engine is deterministic and free of LLM calls.
"""
...
def check_unit_missing(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_unit_invalid(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_quantity_negative(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_no_primary_reactant(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_no_entities(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_solvent_is_reactant(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_product_quantity_as_input(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_temperature_range(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_duration_range(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_pressure_range(recipe: ChemicalRecipe) -> list[ValidationError]:
    """Block pressure values outside physically plausible synthesis ranges."""

    if recipe.conditions is None:
        return []
    pressure = recipe.conditions.pressure_atm
    if pressure is not None and not (0.0001 <= pressure <= 1000.0):
        return [ValidationError(
...
def check_yield_range(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_no_conditions(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_entity_name_too_short(recipe: ChemicalRecipe) -> list[ValidationError]:
...
def check_duplicate_entity_names(recipe: ChemicalRecipe) -> list[ValidationError]:
    """Block repeated entity names because duplicates corrupt graph nodes."""

    seen: set[str] = set()
    errors: list[ValidationError] = []
    for index, name in enumerate(entity.name.lower().strip() for entity in recipe.entities):
        if name in seen:
            errors.append(
                ValidationError(
...
def check_moles_mass_inconsistent(recipe: ChemicalRecipe) -> list[ValidationError]:
